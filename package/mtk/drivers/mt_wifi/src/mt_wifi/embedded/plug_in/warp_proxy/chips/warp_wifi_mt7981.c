 ...
 ***************************************************************************
	...
	warp_wifi_mt7981.c
*/
#include "warp_wifi_mt7981.h"
#include "mac/mac_mt/fmac/mt_fmac.h"
#include "../warp_proxy.h"
#include <warp.h>

static void mt7981_fbuf_init(unsigned char *fbuf, unsigned int pkt_pa, unsigned int tkid)
{
	struct txd_l *txd;
...
static struct wifi_ops mt7981_jedi_ops = {
	.config_atc = client_config_atc,
	.swap_irq = client_swap_irq,
	.fbuf_init = mt7981_fbuf_init,
...
void mt7981_chip_specific_get(struct wifi_hw *hw)
{
...
u32 mt7981_warp_register_client(struct wifi_hw *hw)
{
	return warp_register_client(hw, &mt7981_jedi_ops);
}
